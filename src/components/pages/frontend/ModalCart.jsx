        ...
        {option === "cart" ? (
          <div className="max-w-[600px] w-full bg-white rounded-md absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 overflow-hidden">
            ...
            <div className="modal-main p-2">
              <div
                className={`flex flex-wrap gap-6  max-h-[600px]  custom-scroll overflow-y-auto ${
              ...
              >
                {cartData.length === 0 ? (
                  ...
                  cartData.map((item, key) => (
                    <div
                      className="text-center space-y-1 self-start basis-[30%]"
                      key={key}
                    >
                      <img
                      ...
                      <h5>{item.menu_price}</h5>
                      <ul className="flex items-center gap-3 justify-center">
                        <li>
                          <button
                            className="grid size-[30px] rounded-full place-content-center bg-primary text-white"
            ...
            {cartData.length > 0 && (
              <div className="modal-summary absolute bottom-0 left-0 w-full p-4 bg-white flex justify-between items-center shadow-[0_10px_30px_rgba(0,0,0,_0.4)]">
                <h2 className="mb-0">Total: P {getTotal.toFixed(2)}</h2>
                <button
          ...
          <div className="max-w-[600px] w-full bg-white rounded-md absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 overflow-hidden">
            <div className="modal-header p-4 flex justify-between items-center border-b border-gray-200">
              <button onClick={() => setOption("payment")}>
                <ArrowLeft />
              </button>
